...
export default function WorkspacePanel() {
  ...
  const run = async () => {
    ...
    setError("");
    try {
      const data = await fetchItems({ query: "Alex Kim, 180 lb, lean bulk, 2900 kcal", preferences: ["high-protein", "low-prep"] });
  ...
  return (
    <section className="rounded-lg border border-border bg-card p-4 shadow-soft">
      <div className="flex items-center justify-between">
        ...
        <button onClick={run} className="rounded-md bg-primary px-4 py-2 font-medium text-primary-foreground transition hover:opacity-90">Generate Meal Plan</button>
      </div>
